...
export default function event(state = initialState, action) {
  switch (action.type) {
    case types.MARK_EVENT_SUBMITTED:
      const eventList = state.events.slice(0);
    ...
    case types.UPDATE_EVENT:
      const localEvents = state.events.slice(0);
    ...
    case types.REMOVE_EVENT:
      const newEvent = state.events.slice(0);
    ...
    case types.CREATE_EVENT.SUCCESS:
      const events = state.events ? state.events.slice(0) : [];
